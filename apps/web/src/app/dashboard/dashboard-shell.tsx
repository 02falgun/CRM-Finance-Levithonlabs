...
import { clearSession, UserSession, TenantContext } from '../../lib/auth';
...
export default function DashboardShell({ user, tenant, children }: Props) {
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    ...
    if (item.name === 'Customers') {
      return can('lead:read') || can('invoice:read');
    }
    return can(reqPerm);
  });
  ...
  if (activeItem && activeItem.name !== 'Dashboard') {
    ...
    if (activeItem.name === 'Customers') {
      isUnauthorized = !can('lead:read') && !can('invoice:read');
    } else if (reqPerm) {
      isUnauthorized = !can(reqPerm);
    }
  }
      ...
      <div className="flex h-screen bg-[#F8F8F8] items-center justify-center text-[#111111] p-6 font-sans">
        <div className="text-center space-y-4 max-w-sm bg-white border border-slate-200 p-8 rounded-2xl shadow-sm">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            Your current role profile does not carry permissions required to view the{' '}
            <span className="font-bold">{activeItem?.name}</span> registry.
          </p>
          <button
  ...
  return (
    <div className="flex h-screen bg-[#F8F8F8] overflow-hidden text-[#111111] antialiased font-sans">
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3.5 px-3.5 py-2.5 rounded-lg text-xs font-semibold tracking-wide transition-all ${
                      isActive ? 'bg-[#111111] text-white' : 'text-slate-500 hover:text-[#E86D1F] hover:bg-slate-50'
                    }`}
                  >
                    <item.icon className={`h-4.5 w-4.5 ${isActive ? 'text-[#E86D1F]' : 'text-slate-400'}`} />
                    <span>{item.name}</span>
                  </Link>
                );
              })}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-[#F8F8F8] bg-linear-grid p-6 relative">{children}</main>
      </div>
    </div>
  );
}
